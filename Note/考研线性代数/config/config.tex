...
% 设置列表环境的上下间距
\setenumerate[1]{itemsep=2pt,partopsep=0pt,parsep=\parskip,topsep=2pt}
\setitemize[1]{itemsep=2pt,partopsep=0pt,parsep=\parskip,topsep=2pt}
\setdescription{itemsep=2pt,partopsep=0pt,parsep=\parskip,topsep=2pt}                              %
% 定理环境
% ########## 定理环境 start ####################################
...
\newtcbtheorem[auto counter, number within=section, list type=subsubsection, list inside=toc]{lemma}{总结}
{
    colframe=blue,colback=blue!3!,fonttitle=\bfseries, title={Comment \thetcbcounter}, list entry={Comment \thetcbcounter\quad}, %标题
    breakable, %支持跨页
    before upper={\parindent10pt\noindent},  % 支持缩进。\noindent：首行不缩进
    % left = 2mm, %文字离线框左边的边距
    % right = 1mm,%同上
...
{
    colback=blue!10!white,colframe=blue!50!black,fonttitle=\bfseries, title={Comment \thetcbcounter}, list entry={Comment \thetcbcounter\quad}, %标题
    breakable, %支持跨页
    before upper={\parindent10pt\noindent},  % 支持缩进。\noindent：首行不缩进
    % left = 2mm, %文字离线框左边的边距
    % right = 1mm,%同上
...
{cor}
\newtcbtheorem[auto counter, number within=section, list type=subsubsection, list inside=toc]{conclusion}{结论}
{
    colback=red!5,colframe=red!75!black,fonttitle=\bfseries, title={Comment \thetcbcounter}, list entry={Comment \thetcbcounter\quad}, %标题
    breakable, %支持跨页
    before upper={\parindent10pt\noindent},  % 支持缩进。\noindent：首行不缩进
    % left = 2mm, %文字离线框左边的边距
    % right = 1mm,%同上
...
\ExplSyntaxOn
\clist_set:Nn \l_tmpa_clist {72, 72, 72, 72, 72, 73, 73, 73, 73, 73}
\cs_new:Npn \__ljguo_get_rating:n #1 {
    \int_set:Nn \l_tmpa_int { 6 - #1 }
    \int_set:Nn \l_tmpb_int { 10 - #1 }
    \group_begin:
    \color_select:n { red }
    \int_step_inline:nnn { \l_tmpa_int } { \l_tmpb_int }
    {
        \ding{\clist_item:Nn \l_tmpa_clist { ##1  }}
    }
    \group_end:
}
\let\getRating\__ljguo_get_rating:n
% 评级命令:\getRating{n}
\ExplSyntaxOff